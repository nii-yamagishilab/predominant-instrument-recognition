...
for k, v in tqdm(val_split.items()):
    ...
    if mean_sig < 5e-4:
        with open('nsynth_silent_samples_val.txt','a') as tt:
...
count = 0
for k, v in tqdm(train_split.items()):
    signal, SR = torchaudio.load(
        os.path.join('nsynth_data/nsynth-{}'.format(v['official_split']), 'audio', v['note_str'] + '.wav'))
    signal = signal.squeeze(0)
    mean_sig = torch.mean(np.abs(signal))
    if mean_sig < 5e-4:
        with open('nsynth_silent_samples_train.txt','a') as tt:
            tt.write(v['official_split'] + '||' + v['note_str'] + '\n')
        count += 1
print('[TRAIN]number of quiet samples: {}'.format(count))


with open('nsynth_silent_samples_train.txt','r') as f1:
    for strs in f1.readlines():
        n_strs = strs.strip().split('||')[-1]
        del train_split[n_strs]
with open('nsynth_silent_samples_val.txt','r') as f1:
    for strs in f1.readlines():
        n_strs = strs.strip().split('||')[-1]
