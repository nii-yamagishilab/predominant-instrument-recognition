...
class InstrumentRecognizer(pl.LightningModule):
    ...
    def configure_optimizers(self):
        if self.hparams.optim.lower() == 'adam':
            ...
            optimizer = torch.optim.AdamW(self.parameters(), lr=self.hparams.lr, weight_decay=2e-05)
        else:
            raise NotImplementedError
    ...
    def training_step(self, batch, batch_idx):
        ...
        labels = batch['label']
        fml_labels = batch['fml_label']

        if self.hparams.loss_fn == 'CE':
            if torch.rand(1).item() >= self.hparams.mixup_p:
                _, (outputs, outputs_b) = self(signals)
                loss = self.loss(outputs, labels)
                loss += self.loss(outputs_b, fml_labels)
            else:
                mixup_style_list = ['mixup']
                mixup_style = random.choice(mixup_style_list)
                if mixup_style == 'mixup':
                    mixed_signals, y_a, y_a_roll, y_b, y_b_roll, lam = self.mixup_data(signals, labels, fml_labels, alpha=self.hparams.mixup_alpha)
                elif mixup_style == 'cutmix':
                    mixed_signals, y_a, y_b, lam = self.cutmix_data(signals, labels,
                                                                    alpha=1.0)
                else:
                    raise NotImplementedError
                _, (outputs, outputs_b) = self(mixed_signals)
                loss = lam * self.loss(outputs, y_a) + (1 - lam) * self.loss(outputs, y_a_roll)
                loss += lam * self.loss(outputs_b, y_b) + (1 - lam) * self.loss(outputs_b, y_b_roll)
                # else:
                #     mixed_signals, y_list, lam_list = self.multi_mixup(signals, labels)
                #     _, outputs = self(mixed_signals)
                #     loss = lam_list[0] * self.loss(outputs, y_list[0])
                #     for ii in range(1, len(lam_list)):
                #         loss += lam_list[ii] * self.loss(outputs, y_list[ii])
    ...
    def mixup_data(self, x, y, y2,  alpha=1.0):
        ...
        y_a, y_a_roll, y_b, y_b_roll = y, y_roll , y2, y2_roll
        return mixed_x, y_a, y_a_roll, y_b, y_b_roll, lam
